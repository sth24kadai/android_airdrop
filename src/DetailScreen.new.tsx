...
export default class DetailScreen extends ShardSender<"DetailScreen"> {
    ...
    onButtonPress(
        service: Service & { clientName: string; clientModel: string }
    ) {
        if (service === null) {
            this.context.logs.push({
                emoji: "🤬",
                message: "サービスが選択されていません。",
            });
            return;
        }
                ...
                <SafeAreaView style={styles.container}>
                    <RNScrollView>
                        <Button
                            ...
                            disabled={
                                this.state.isSending || !this.context.image
                            }
                        >
                            データを送信する
                        </Button>
                        ...
                        {this.state.isSending && (
                            <View style={styles.statusComponent}>
                                ...
                                <PaperText>
                                    推定残り時間:{" "}
                                    {Math.round(this.state.estimate / 1000)}秒
                                </PaperText>
                            </View>
                        )}
                    </RNScrollView>
